...
const EditGigForm: React.FC<PostGigProps> = ({ formValues, onFormChange, onNext }) => {
    ...
    const validateForm = () => {
        ...
        const trimmedProjectName = formValues.projectName.trim();
        const trimmedDescription = formValues.description.trim();
        const trimmedSearchTags = formValues.searchTags.map((tag: string) => tag.trim());
    
        if (trimmedProjectName.length < 3) {
            newErrors.projectName = "Project name must be at least 3 characters long and not just whitespace.";
        }
        if (trimmedDescription.length < 10) {
            newErrors.description = "Description must be at least 10 characters long and not just whitespace.";
        }
        if (trimmedSearchTags.length < 2 || trimmedSearchTags.some(tag => tag.length === 0)) {
            newErrors.searchTags = "At least 2 valid tags are required, and they cannot be empty.";
        }
